...
public class Manager extends Handler {
    ...
    private final Queue<NotificationView> notifyQueue;

    private boolean isSticky=false;

    private Manager() {
        notifyQueue = new LinkedBlockingQueue<NotificationView>();
    }
    ...
    void add(NotificationView crouton,boolean repeat ) {
    ...
    void addSticky(NotificationView crouton) {
    ...
    void removeSticky(){
        NotificationView notify = notifyQueue.peek();
        if(notify!=null) {
    ...
    private long calculateCroutonDuration(NotificationView notify) {
    ...
    private void sendMessage(NotificationView crouton, final int messageId) {
        final Message message = obtainMessage(messageId);
        message.obj = crouton;
        sendMessage(message);
    }

    private void sendMessageDelayed(NotificationView crouton, final int messageId, final long delay) {
    ...
    public void handleMessage(Message msg) {
        final NotificationView notify = (NotificationView) msg.obj;
        if (null == notify) {
            return;
        }
        switch (msg.what) {
    ...
    private void displayNotify(boolean sticky) {
        ...
        final NotificationView currentNotify = notifyQueue.peek();
    ...
    private void addNotifyToView(final NotificationView notify) {
        if (notify.isShowing()) {
            return;
        }
    ...
    protected void removeNotify(NotificationView notify) {
    ...
    protected void removeNotifyView(NotificationView notify) {
        View notifyView = notify.getView();
        ViewGroup notifyParentView = (ViewGroup) notifyView.getParent();
        if (null != notifyParentView) {

            NotificationView removed = notifyQueue.poll();
